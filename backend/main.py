...
@app.get('/api/status')
def status(camera_model: str | None = Query(default=None)) -> dict[str, Any]:
    state = runtime.last_state or runtime.monitor.collect()
    camera_status: dict[str, Any]
    try:
        camera_status = runtime.camera.camera_status(camera_model)
    except Exception as exc:
        camera_status = {'connected': False, 'error': str(exc), 'model': camera_model or runtime.camera.default_model}
...
def camera_record_start(model: str | None = Query(default=None)) -> dict[str, Any]:
    try:
        return runtime.camera.start_recording(model)
    except Exception as exc:
        raise HTTPException(status_code=500, detail=str(exc)) from exc


@app.post('/api/camera/record/stop')
def camera_record_stop(model: str | None = Query(default=None)) -> dict[str, Any]:
    try:
        return runtime.camera.stop_recording(model)
    except Exception as exc:
        raise HTTPException(status_code=500, detail=str(exc)) from exc


@app.post('/api/camera/photo')
def camera_photo(model: str | None = Query(default=None)) -> dict[str, Any]:
    try:
        return runtime.camera.take_photo(model)
    except Exception as exc:
        raise HTTPException(status_code=500, detail=str(exc)) from exc
